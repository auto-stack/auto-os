...
- **几何吸附（PLAN-035 SD-01 修订：8×3 定框 + 网格算术卡位）**：面板
  ...
  裁定（原 10 列框 + 内部三等分 sx/sy 缩放退役）。头行 = 网格行 0 满格
  （72px + gap 8，`DASH_HEADER_H`=80）；face 卡 = 网格单元整数倍：
  **缺省 2×2**（168×152）、声明/存储 **3×2**（256×152），卡框线落
  88/80 节距网格线；`dashboard_layout` 直出视口绝对矩形（外框 + 格位
  同一算式，chrome wrapper 与 face 卡永远同源）。8 列单卡行装不下者
  裁剪 + dev 日志（滚动/增高挂平板网格 v2，PLAN-024 §9.3）。
