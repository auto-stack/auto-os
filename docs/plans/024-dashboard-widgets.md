...
| R17 | sysmon 卡加分档栅格进度条（CPU/内存；绿→蓝→黄→红随值变档） | store tick 百分比 int 化 + 20 段 seg 对象表 + 档位色（插值消费）；mini 视图 CPU/内存行下插段条；Tick 驱动、R5 门控照常（不看零开销）。坑：seg 对象键 `on` 撞 .at 关键字 → 改 `lit` | auto-os R17/R17b/R17c 提交 | ✅（待用户终审） | 实机：系统 tab 多色栅格条 |
