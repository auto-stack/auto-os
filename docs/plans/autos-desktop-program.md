...
| **M7-c app 组合批量 a2r** ① ✅ **已交付（PLAN-039，2026-09-21）——① 近邻五 app** | ①面内近邻 = klondike（icon/card）/ minesweeper（grid）/ kanban（input/textarea）/ launcher（icon/mouse-area/input/grid）/ tetris（dialog 折叠形态核对）——**已交付**：生成门矩阵收敛（klondike 190→0 / launcher 56→0 / minesweeper 0 零回归 / kanban 唯一红=ondrop×3 D3-A 显式拒绝；tetris 0）+ 批次 E store 动态语义译臂（内建方法表/降链家族/记录形状注册表/back `pub type` struct 发射/无类型局部联合格/组件传型七面）+ 发现臂⑦件（递归收集/key 同弃/字面量再转义/动态标签 format!/target 幻影废除/ARIA 族同弃/pac 注释 `#[` 毒行/P037 关联）+ desktop_exe 四声明（kanban 不声明=pac 无 desktop 面+drag 债注记）+ launcher 编译轨（spawn_launcher_outproc 编译产物优先=P036-D2 核销面 + key_message 产物面=P036-D4）+ 三轨验收（klondike Vue 三套绿+VM 26/26+Rust 金样 11+1 诚实红[F-04a 深树快照债]；tetris vm_rules_golden 恢复 PASS+run_matrix rust 腿 unblock）；②tabs 户 = jade-garden（tab×27）/ auto-musk（tab×16 + alert-dialog×7 + table×1，17821 行拆批）——**依赖 M7-a tabs**；③table 族 = sys-monitor（table 全家 + svg + alert-dialog×16，table kind 捆绑立项）；特殊线 auto-term（terminal kind）；**②③延期裁定（2026-09-21，用户）**：jade-garden 已全面改组——旧版纯 Vue 实现保留不动，新版更名 jade-edit 迁独立目录 `../jade-edit` 以 Auto 重开发（与 auto-edit 共享架构），VM 版能用前不考虑 a2r；auto-musk VM 版功能未完善，等待；sys-monitor 样式需大改，等待——三件均延后暂缓立项，M7-d 与 pixels 退役门覆盖收口条件随之顺延 | —（逐 app codegen 缺口即发现即修，缺口按 app 记账——本批：F-04a 编译 exe 深树 MCP 快照仅根节点[RqProjector 面]/kanban drag not-yet 家族/p036 e2e 复跑挂 T-10[ui-iced 档基线归因]） | ①已达成（上列）；②③**延期**（2026-09-21 裁定，见内容列） |
| **M7-d 全 a2r 桌面收口** | 整桌面度量（对照 508 inproc 0.86 / 020 queue 2.42 MiB/App 基线）+ 全链 smoke + 终态入册（Design 23 里程碑收笔） | — | 度量报告 + 终态验收留痕 |

**顺序**：M7-a → {M7-b, M7-c①} 可并行（文件面错开：投影器/覆盖表 ‖ 生成器/宿主装配 ‖ app 仓）→ M7-c②③（②依赖 M7-a 的 tabs；**②③已延期 2026-09-21**，见 M7-c 行）→ M7-d 压轴（随②③顺延）。
...
046 真源编译过）——jade-garden（tab×27）/auto-musk（tab×16）可开工（**2026-09-21 裁定延期**，见 M7-c 行）；
...
**M7 副线裁定交付行（PLAN-037，2026-09-20）**：desktop back provision（launch 后端供给机制）**已交付**——VM 桌面（inproc 直挂）launch app 此前后端零供给（020 打开即空曲库）；本件把 658 的 back proxy 承载层原样引进桌面宿主，生命周期自画廊 "boot 期全量静态注册" 翻转为 "**launch 时按需装载、窗关卸载**"：launch 决策树四臂单一裁决点（①pac daemon → 既有 ensure 链真进程不进 proxy / ②pac media_root → proxy 原生 media 路由 / ③back api.at 特形谓词[~Stream/~Promise/行首 use auto.——658 谓词迁 auto-lang 双消费] → proxy VM session / ④纯前端零供给；②③可叠加）+ 懒启门（boot 零常驻——首个命中才 start，AC-06）+ app_key 单例引用计数（双窗共享一份后端，四关窗站点统一 release 归零即卸载——drop sender 线程自然退出）+ 前端 URL **内存态**前缀化（不落盘不改语料；origin-only root + 模块粒度 launch 作用域 overlay[back_prefix.rs]——async HTTP base 解析在独立线程属进程 env 架构，请求期 per-app base 架构性不可行，源级变换为终选）。规范增量：lang docs/specs/auto-lang/vm/back-proxy.md「运行期增删与生命周期」节（SD-01）。验收：020 真实曲库 393 条目入 store + 流 206 audio/mpeg（smoke-037 真管道）；cargo t 零新增红（master 对拍）；画廊/独立形态照旧。真机播放验收归用户（ToDesk 合成输入约束）。
