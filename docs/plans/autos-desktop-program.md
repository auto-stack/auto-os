...
**副线债**（不阻塞终态，按需排期）：双投影器统一（P020-D1，统一时更名 RqProjector——2026-09-19 用户意向）；**pixels 臂退役门**（2026-09-19 用户裁定细化，拆两半）：**解释半臂提前退役**（不等位图过线——`run_independent_child` + `process_model=outproc` 解释隔离选项一并废除：解释态 app 一律内嵌直挂[虚拟桌面/ui-gallery 同律]，隔离渲染初心=渲染器外移省钱，自渲再发帧违背初衷；连锁 = AppProjector 解释 queue 臂与 `-q` VM 轨存废待裁定，P020-D1 或改写为"解释臂退役+RqProjector 独存"）；native 半臂维持四条件（auto 翻转[032] / 覆盖收口[M7-c] / 位图过线 / 像素原生族裁定——Linux 无强制场景：iced+wgpu 自渲 app 经 swapbuffers 呈递 OS compositor 属系统协议非本协议臂）；**量化验收门：单 app `-q` 渲染正确 + app ≤10MB[031 实测 7-9MB ✅] + rqhost ≤100MB[031 debug 实测 314MB ❌，release 复测+归因优化为前置]**；达标后执行退役（P-RQ-PIX 在册）；位图过线通道（app 动态位图 + web 真位图）；518 色彩上下文；GUI 级 OS 自动化（P020-D4 点击面）；Stage B 搬迁（桌面壳代码 auto-lang→auto-os）；L3 StateSnapshot native 注入；imagesurface 交互族 / svgdoc 词汇；rqhost 生态（`--rq-host=desktop` attach 等）。
