...
**修正裁定先行**（较 Plan 584 草案假设）：草案曾把 shell 四件
`crates/auto-lang/assets/{shell,desktop,switcher,notification_center}.at` 列为
核心随迁资产；实测发现它们被框架 **`include_str!` 编译期内嵌**
（`crates/auto-lang/src/ui/shell.rs:10/21/32/45` 四个 const），且框架代码与其
内容共演化——renderer/layout/session 侧硬编码了源自 shell.at 的契约（任务栏
行高 56px 对齐、`verb\u{1F}arg` 记录编码、dock hover 语义等）。整体外迁 =
框架去内嵌改造 + 全量桌面测试合同重锚，超出 Stage B 范围。**裁定：shell 四件
留架为框架内嵌资产（入 §2 表），本仓登记指针；「shell pack 路径化覆盖
（env 指定外部 shell 皮肤目录，内嵌为回退）」列 Stage C 演进候选（§7）。**
...
| L2 | shell 四件 .at | `crates/auto-lang/assets/{shell,desktop,switcher,notification_center}.at` | 编译期内嵌资产，与 L1 共演化（§1 修正裁定） | Stage C「shell pack 路径化覆盖」演进候选（§7）；本仓登记指针 |
...
两项均为 Stage B **硬前置**（框架仓改动，Category B 作用域），各立小 plan、
互不依赖、不依赖搬迁时机。R-D1 状态修正（较 Plan 584 草案）：583 合并提交
...
| **P-5** | 资产搬迁本体批：A1 台账 + A3 launcher + B 批 apps + common 抽取 + L2/L8 指针登记 + 框架层行数实测归档（tokei/cloc） | 两仓 | P-2/P-3 落地（搬完即可跑）；P-4 建议先行 | **541/582 合并后**（唯一硬窗口） |
| **P-6** | 随迁计划开工：七项在 auto-os 逐个执行（首个建议 554 或 577——小面验证解析序与流程） | auto-os | P-1 + P-5 | — |
| Stage C 候选 | shell pack 路径化覆盖（解 L2 内嵌）；`auto desktop` 一等子命令（§3-b）；auto-cosmic 归属重估（L4）；app 升格独立仓 17xxx 带规约化 | — | Stage B 稳定后 | — |

P-2/P-3/P-4 三者互不依赖、不依赖 P-1/P-5 时机，设计定案后即可并行立项。
