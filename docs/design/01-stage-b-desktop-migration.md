...
| **P-6** | 随迁计划开工：七项在 auto-os 逐个执行（首个建议 554 或 577——小面验证解析序与流程）+ §3-a 包装脚本 + P-5 随迁资产 V1/V2/V3 实机验收 + auto-os 侧画廊变更触发 website 部署（repository_dispatch）——**🔄 承载批已落地（2026-09-08，PLAN-009 T1–T7 execution_done 待复审）：V8 冒烟=os-003 开工 T1 ✅（解析序路径全可达+vue 轨 running 门控差发现）；wrapper 双脚本+V5 38/22 重证（含 cargo CWD 栈旗标教训）；V2 零漂移（025=13/0〔541 基线〕/028=24/0〔464〕/038=22/0，框架侧 calculator/charts 两预存红债候选）；V3 双端三截图；V1 五面实机（launcher→038 启动端到端）；R4 CI 围栏真跑（lang `018ec4325`）；画廊触发端双侧（token 待配，降级在案）。七项本体执行在途：os-003 executing，余六项 drafting 排队** | auto-os | P-1 + P-5 | — |
