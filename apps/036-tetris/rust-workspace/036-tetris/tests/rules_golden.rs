...
#[test]
fn all_piece_rotations_golden() {
    let store = fresh_playing();
    let expected: [[i32; 8]; 28] = [
        [0, 1, 1, 1, 2, 1, 3, 1],
        [2, 0, 2, 1, 2, 2, 2, 3],
        [0, 2, 1, 2, 2, 2, 3, 2],
        [1, 0, 1, 1, 1, 2, 1, 3],
    ...
    assert_eq!(store.shapes.len(), expected.len());
    for (index, shape) in expected.iter().enumerate() {
        assert_eq!(store.shapes[index], shape.to_vec(), "shape rotation {index}");
    }
}
